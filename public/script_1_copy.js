  ...
  gateWays = collection1.map((obj) => {
   ...
   allFlights = collection2.map((obj) => {
      return {
        ...
        marker : null,
        circle : null,
        collisionTime : new IncrementingObject()
      };
    //}
...
function initMap() {
  ...
  /*class circleMake{
    constructor (lt, lg){
      this.circle = new google.maps.Circle({
        ...
        radius: 100000 // Set the radius in meters
      });
    }
    blinkCircle() {
      var showCircle = true;
      ...
      }, 500); // Toggle visibility every 500 milliseconds (adjust as needed)
    
      // Clear the blink interval after the specified duration
      setTimeout(function() {
        clearInterval(blinkInterval);
        this.circle.setVisible(false); // Hide the circle after the blinking is complete
      }, 8000);
    }
  ...
  //------------------------ testing  blink marker 2023.05.31
  
  //blinkCircle(circle, 50000);
  /*var interval11 = setInterval(function() {
    collidedFlights.push(new circleMake(getRandomNumber(-90, 90), getRandomNumber(-180, 180)));
    collidedFlights[collidedFlights.length-1].blinkCircle();

  }, 4000)*/

  //------------------------ testing  blink marker end 
          ...
          for(let p = 0; p < compArr.length; p++){
            for(let pInner = 0; pInner < compArr.length; pInner++){
              ...
              if (distance < radius) {
                ...
                //collidedFlights.push(compArr[p]);
                //collidedFlights.push(compArr[pInner]);
                flightInfo.find(obj => obj.Callsign === compArr[p].Callsign).circle = createCollisionCircle(flightInfo.find(obj => obj.Callsign === compArr[p].Callsign).marker.getPosition());
                flightInfo.find(obj => obj.Callsign === compArr[p].Callsign).circle.setMap(map);
                //console.log(flightInfo.find(obj => obj.Callsign === compArr[p].Callsign).circle.label);
