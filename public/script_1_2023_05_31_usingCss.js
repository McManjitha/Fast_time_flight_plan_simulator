...
function initMap() {
  ...
  class CustomOverlay extends google.maps.OverlayView {
    constructor(lt, lg, element) {
      super();
      this.position = { lat: lt, lng: lg };
    ...
    draw() {
      ...
      if (!overlayProjection || !this.element) return;

      const positionOnMap = overlayProjection.fromLatLngToDivPixel(this.position);

      this.element.style.position = "absolute";
      this.element.style.left = positionOnMap.x + "px";
      this.element.style.top = positionOnMap.y + "px";
    }
          ...
          for(let p = 0; p < compArr.length; p++){
            for(let pInner = 0; pInner < compArr.length; pInner++){
              ...
              if (distance < radius) {
                ...
                collidedPoints.push(new CustomOverlay(compArr[p].lat, compArr[p].lng, divElement));
  ...
  intervalId3 = setInterval(function() {
    if(collidedPoints.length){
      for(let point = 0; point < collidedPoints.length; point++){
        collidedPoints[point].setMap(map);
        collidedPoints[point].element.classList.toggle("hidden")
        collidedPoints[point].count++;
        if(collidedPoints[point].count > 8){
          collidedPoints[point].setMap(null);
